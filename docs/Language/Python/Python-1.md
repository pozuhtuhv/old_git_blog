...
```html
단순히 몇일 requests, selenium 공부했다고 뚝딱뚝딱 만들어 냈지만, 
다음페이지가 있던 없던 첫페이지만 가져오게 만든 코드다.
물론 첫페이지에 원하던 결과가 나왔었으니 이렇게 만든것 같다.

지금 다시 만든다고 하면 좀 더 개선 될지는 몰라도
이때 처음 만들었을때는 ChatGPT 가 없었을 때니깐..
찾는데도 한참을 걸렸던 것으로 기억한다.
```